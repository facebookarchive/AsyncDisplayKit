...
- (void)setChild:(id<ASLayoutable>)child atIndex:(NSUInteger)index
{
  ...
  id<ASLayoutable> layoutable = child ? [self layoutableToAddFromLayoutable:child] : nil;
  ...
  [_childrenArray addPointer:(__bridge void *)layoutable];
...
- (id<ASLayoutable>)childAtIndex:(NSUInteger)index
{
  ...
  if (index < _childrenArray.count) {
    layoutable = (__bridge id<ASLayoutable>)[_childrenArray pointerAtIndex:index];
  }
  
  // Null layoutable should not be accessed
  ASDisplayNodeAssertNotNil(layoutable, @"Access child at index without set a child at that index");
  return layoutable;
}

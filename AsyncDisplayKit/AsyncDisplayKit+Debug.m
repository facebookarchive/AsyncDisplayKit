...
{
  NSString *debugString = [NSString stringWithFormat:@"%lu N %lu L", (long)info.nodeCount, (long)info.layerCount];
  
  // only show # of views if different from # of nodes
  if (info.nodeCount != info.viewCount) {
    debugString = [debugString stringByAppendingString:[NSString stringWithFormat:@" %lu V", (long)info.viewCount]];
  }

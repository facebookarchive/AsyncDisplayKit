...
- (void)_loadViewOrLayerIsLayerBacked:(BOOL)isLayerBacked
{
...

  {
    ...
    if (_loadBlock) {
      ...
      Class viewOrLayerClass = [viewOrLayer class];

      if ([viewOrLayerClass isSubclassOfClass:[CALayer class]]) {
        _layer = viewOrLayer;
        [self _initializeLayerClass:viewOrLayerClass];
      }

      else if ([viewOrLayerClass isSubclassOfClass:[UIView class]]) {
        _view = viewOrLayer;
        [self _initializeViewClass:viewOrLayerClass];
      }

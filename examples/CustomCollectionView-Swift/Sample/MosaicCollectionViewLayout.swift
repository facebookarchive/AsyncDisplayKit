...
class MosaicCollectionViewLayout: UICollectionViewFlowLayout {
  ...
  override func prepare() {
    ...
    let numberOfSections: NSInteger = self.collectionView!.numberOfSections
    ...
    for section in 0 ..< numberOfSections {
      ...
      if (headerHeight > 0) {
        ...
        let attributes: UICollectionViewLayoutAttributes = UICollectionViewLayoutAttributes.init(forSupplementaryViewOfKind: UICollectionElementKindSectionHeader, with: NSIndexPath.init(row: 0, section: section) as IndexPath)
        
        attributes.frame = CGRect.init(x: _sectionInset.left, y: top, width: headerSize.width, height: headerSize.height)
      ...
      let columnWidth: CGFloat = self._columnWidthForSection(section: section)
      _itemAttributes?.append([])
      for idx in 0 ..< numberOfItems! {
        let columnIndex: Int = self._shortestColumnIndexInSection(section: section)
        let indexPath = IndexPath.init(item: idx, section: section)
        
        let itemSize: CGSize = self._itemSizeAtIndexPath(indexPath: indexPath);
        let xOffset: CGFloat = _sectionInset.left + (columnWidth + columnSpacing) * CGFloat(columnIndex)
        let yOffset: CGFloat = _columnHeights![section][columnIndex]
  ...
  {
    var size: CGSize = CGSize.init(width: self._columnWidthForSection(section: indexPath.section), height: 0)
    let originalSize: CGSize = self.delegate!.collectionView(self.collectionView!, layout:self, originalItemSizeAtIndexPath:indexPath)
  ...
  {
    return CGSize.init(width: self._widthForSection(section: section), height: headerHeight)
  }
  ...
  override var collectionViewContentSize: CGSize
  {
    ...
    return CGSize.init(width: self.collectionView!.bounds.size.width, height: height)
  }
...
class MosaicCollectionViewLayoutInspector: NSObject, ASCollectionViewLayoutInspecting
{
  ...
  func collectionView(_ collectionView: ASCollectionView, numberOfSectionsForSupplementaryNodeOfKind kind: String) -> UInt {
    if (kind == UICollectionElementKindSectionHeader) {
      return UInt((collectionView.asyncDataSource?.numberOfSections!(in: collectionView))!)
    } else {
      return 0
    }
  }
